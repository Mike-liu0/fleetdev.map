...
public partial class AddNewOperation : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        Sycamore_Operation package_item = new Sycamore_Operation();
        ...
        if (!Config.CheckKey(key))    // Check failed
        {
            response.AppendFormat("{{\"success\":\"false\", \"message\":\"{0}\", \"package\":{1}}}",
                "Authentication fail", Sycamore_Operation.ToJson(package_item));    //Write response into string
        ...
        if (output == "output")
        {
            response.AppendFormat("{0}", Sycamore_Operation.ToJson(package_item));
            Response.Write(response.ToString());
            return;
        }
        ...
        {
            response.AppendFormat("{{\"success\":\"false\", \"message\":\"{0}\", \"package\":{1}}}",
                "Data input fail", Sycamore_Operation.ToJson(package_item));
            Response.Write(response.ToString());
            return;
        }
        ...
        try
        {
            ...
            package.Replace("\\", "");
            Sycamore_Operation item = Sycamore_Operation.FromJson(package);

            string result = aWSController.OnDynamoDB_AddNewOperation(item);
            Guid guidResult;
            ...
            if (isValid)
            {
                response.AppendFormat("{{\"success\":\"true\", \"message\":\"{0}\", \"package\":{1}}}",
                    "API.AddNewOperation Complete", Sycamore_Operation.ToJson(item));
            ...
            {
                response.AppendFormat("{{\"success\":\"false\", \"message\":\"{0}\", \"package\":{1}}}",
                    "Database Error: " + result, Sycamore_Operation.ToJson(package_item));
        ...
        {
            response.AppendFormat("{{\"success\":\"false\", \"message\":\"{0}\", \"package\":{1}}}",
                "API Error: " + ex.Message, Sycamore_Operation.ToJson(package_item));
