...
public partial class GetAllOperationQRs : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            string result = new AWSController().OnDynamoDB_GetAllOperationQRs(tableName, filterName, filterValue);

            if (result != "[]")
            {
                response.AppendFormat("{{\"success\":\"true\", \"message\":\"{0}\", \"package\":{1}}}",
                     "API.GetAllOperationQRs Complete", result);
