...
public partial class AddNewOperationQR : System.Web.UI.Page
{
    public string OUTPUT = Sycamore_OperationQR.ToJson(new Sycamore_OperationQR());
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!Config.CheckKey(key))    // Check failed
        {
            response.AppendFormat("{{\"success\":\"false\", \"message\":\"{0}\", \"package\":{1}}}",
                "Authentication fail", Sycamore_OperationQR.ToJson(package_item));    //Write response into string
        ...
        try
        {
            ...
            package.Replace("\\", "");
            Sycamore_OperationQR item = Sycamore_OperationQR.FromJson(package);


            string result = new AWSController().OnDynamoDB_AddNewOperationQR(item);
            Guid guidResult;
            ...
            if (isValid)
            {
                response.AppendFormat("{{\"success\":\"true\", \"message\":\"{0}\", \"package\":{1}}}",
                    "API.AddNewOperationQR Complete", Sycamore_OperationQR.ToJson(item));
