...
public partial class DeleteTrackingRecord : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        try
        {
            string result = new AWSController().OnDynamoDB_DeleteTrackingRecordByGuid(guid);

            if (result == "success")
            {
                response.AppendFormat("{{\"success\":\"true\", \"message\":\"{0}\", \"package\":{1}}}",
                    "API.DeleteTrackingRecordByGuid  Complete", Sycamore_TrackingRecord.ToJson(package_item));
