...
{
    public string OUTPUT = Sycamore_Vehicle.ToJson(new Sycamore_Vehicle());
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!Config.CheckKey(key))    // Check failed
        {
            response.AppendFormat("{{\"success\":\"false\", \"message\":\"{0}\", \"package\":{1}}}",
                "Authentication fail", Sycamore_Vehicle.ToJson(package_item));    //Write response into string
        ...
        try
        {
            ...
            package.Replace("\\", "");
            Sycamore_Vehicle item = Sycamore_Vehicle.FromJson(package);


            string result = new AWSController().OnDynamoDB_AddNewVehicle (item);
            Guid guidResult;
            ...
            if (isValid)
            {
                response.AppendFormat("{{\"success\":\"true\", \"message\":\"{0}\", \"package\":{1}}}",
                    "API.AddNewVehicle Complete", Sycamore_Vehicle.ToJson(item));
        ...
        {
            response.AppendFormat("{{\"success\":\"false\", \"message\":\"{0}\", \"package\":{1}}}",
                "API Error: " + ex.Message, Sycamore_Vehicle.ToJson(package_item));
