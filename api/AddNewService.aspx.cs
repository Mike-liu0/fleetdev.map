...
{
    public string OUTPUT = Sycamore_Service.ToJson(new Sycamore_Service());
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        Sycamore_Service package_item = new Sycamore_Service();

        //Security check
        if (!Config.CheckKey(key))    // Check failed
        {
            response.AppendFormat("{{\"success\":\"false\", \"message\":\"{0}\", \"package\":{1}}}",
                "Authentication fail", Sycamore_Service.ToJson(package_item));    //Write response into string
        ...
        if (output == "output")
        {
            response.AppendFormat("{0}", Sycamore_Service.ToJson(package_item));
            Response.Write(response.ToString());
            return;
        }
        ...
        {
            response.AppendFormat("{{\"success\":\"false\", \"message\":\"{0}\", \"package\":{1}}}",
                "Data input fail", Sycamore_Service.ToJson(package_item));
            Response.Write(response.ToString());
            return;
        }
        ...
        try
        {
            ...
            package.Replace("\\", "");
            Sycamore_Service item = Sycamore_Service.FromJson(package);


            string result = new AWSController().OnDynamoDB_AddNewService (item);
            Guid guidResult;
            ...
            if (isValid)
            {
                response.AppendFormat("{{\"success\":\"true\", \"message\":\"{0}\", \"package\":{1}}}",
                    "API.AddNewService Complete", Sycamore_Service.ToJson(item));
            ...
            {
                response.AppendFormat("{{\"success\":\"false\", \"message\":\"{0}\", \"package\":{1}}}",
                    "Database Error: " + result, Sycamore_Service.ToJson(package_item));
        ...
        {
            response.AppendFormat("{{\"success\":\"false\", \"message\":\"{0}\", \"package\":{1}}}",
                "API Error: " + ex.Message, Sycamore_Service.ToJson(package_item));
