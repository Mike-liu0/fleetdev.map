...
public partial class AddNewDriverRate : System.Web.UI.Page
{
    public string OUTPUT = Sycamore_DriverRate.ToJson(new Sycamore_DriverRate());
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!Config.CheckKey(key))    // Check failed
        {
            response.AppendFormat("{{\"success\":\"false\", \"message\":\"{0}\", \"package\":{1}}}",
                "Authentication fail", Sycamore_DriverRate.ToJson(package_item));    //Write response into string
        ...
        try
        {
            ...
            package.Replace("\\", "");
            Sycamore_DriverRate item = Sycamore_DriverRate.FromJson(package);

            
            string result = new AWSController().OnDynamoDB_AddNewDriverRate(item);
            Guid guidResult;
            ...
            if (isValid)
            {
                response.AppendFormat("{{\"success\":\"true\", \"message\":\"{0}\", \"package\":{1}}}",
                    "API.AddNewDriverRate Complete", Sycamore_DriverRate.ToJson(item));
