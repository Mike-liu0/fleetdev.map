...
{
    public string OUTPUT = Sycamore_Driver.ToJson(new Sycamore_Driver());
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!Config.CheckKey(key))    // Check failed
        {
            response.AppendFormat("{{\"success\":\"false\", \"message\":\"{0}\", \"package\":{1}}}",
                "Authentication fail", Sycamore_Driver.ToJson(package_item));    //Write response into string
        ...
        try
        {
            ...
            package.Replace("\\", "");
            Sycamore_Driver item = Sycamore_Driver.FromJson(package);

            
            string result = new AWSController().OnDynamoDB_AddNewDriver(item);
            Guid guidResult;
            ...
            if (isValid)
            {
                response.AppendFormat("{{\"success\":\"true\", \"message\":\"{0}\", \"package\":{1}}}",
                    "API.AddNewDriver Complete", Sycamore_Driver.ToJson(item));
