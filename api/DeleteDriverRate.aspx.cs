...
public partial class DeleteDriverRate : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        try
        {
            string result = new AWSController().OnDynamo_DeleteDriverRateByGuid(guid);

            if (result == "success")
            {
                response.AppendFormat("{{\"success\":\"true\", \"message\":\"{0}\", \"package\":{1}}}",
                    "API.DeleteDriverByGuid  Complete", Sycamore_DriverRate.ToJson(package_item));
