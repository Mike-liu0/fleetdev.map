...
public partial class DeleteOperation : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        try
        {
            string result = new AWSController().OnDynamoDB_DeleteOperationByGuid(guid);

            if (result == "success")
            {
                response.AppendFormat("{{\"success\":\"true\", \"message\":\"{0}\", \"package\":{1}}}",
                    "API.DeleteOperationByGuid  Complete", Sycamore_Operation.ToJson(package_item));
