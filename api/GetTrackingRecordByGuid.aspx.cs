...
public partial class GetTrackingRecordByGuid : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        try
        {
            string result = new AWSController().OnDynamoDB_GetTrackingRecordByGuid(guid);

            if(result != "none")
            {
                response.AppendFormat("{{\"success\":\"true\", \"message\":\"{0}\", \"package\":{1}}}",
                    "API.GetTrackingRecordByGuid Complete", result);
