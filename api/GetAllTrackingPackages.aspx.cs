...
public partial class GetAllTrackingPackages : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            string result = new AWSController().OnDynamoDB_GetAllTrackingPackages(tableName, filterName, filterValue);

            if (result != "[]")
            {
                response.AppendFormat("{{\"success\":\"true\", \"message\":\"{0}\", \"package\":{1}}}",
                     "API.GetAllTrackingPackages Complete", result);
