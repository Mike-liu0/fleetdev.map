...
{
    public string OUTPUT = Sycamore_Report.ToJson(new Sycamore_Report());
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!Config.CheckKey(key))    // Check failed
        {
            response.AppendFormat("{{\"success\":\"false\", \"message\":\"{0}\", \"package\":{1}}}",
                "Authentication fail", Sycamore_Report.ToJson(package_item));    //Write response into string
        ...
        try
        {
            ...
            string result = new AWSController().OnDynamoDB_AddNewReport(item);
            Guid guidResult;
            ...
            if (isValid)
            {
                response.AppendFormat("{{\"success\":\"true\", \"message\":\"{0}\", \"package\":{1}}}",
                    "API.AddNewReport Complete", Sycamore_Report.ToJson(item));
