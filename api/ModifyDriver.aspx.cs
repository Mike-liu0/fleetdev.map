...
public partial class ModifyDriver : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            Guid guidResult;
            string result = new AWSController().OnDynamoDB_ModifyDriver(item);
            ...
            if (isValid)
            {
                response.AppendFormat("{{\"success\":\"true\", \"message\":\"{0}\", \"package\":{1}}}",
                    "API.UpdateDriver Complete", Sycamore_Driver.ToJson(item));
            ...
            {
                response.AppendFormat("{{\"success\":\"false\", \"message\":\"{0}\", \"package\":{1}}}",
                    "Database Error: " + result, Sycamore_Driver.ToJson(item));
